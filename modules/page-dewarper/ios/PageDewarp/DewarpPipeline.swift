...
public class DewarpPipeline {
    ...
    public struct DewarpOutput {
        /// Color-dewarped image (preserves original colors, for display).
        public let colorImage: UIImage
        /// BW adaptive-thresholded image (optimized for OCR).
        public let bwImage: UIImage
    }
    ...
    /// - Returns: `.success(DewarpOutput)` with color and BW dewarped images, or `.failure(DewarpError)`.
    public static func process(image: UIImage) -> Result<DewarpOutput, DewarpError> {
        ...
        // Step 15: remap image (produces both color and BW outputs).
        // Ported from image.py:136 → threshold
        let remap = RemappedImage(img: image, pageDims: pageDims, pvec: params)
        return .success(DewarpOutput(colorImage: remap.colorImage, bwImage: remap.bwImage))
    }
}
